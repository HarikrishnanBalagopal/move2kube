...
func (t *Starlark) Transform(
	...
	_, err := starlark.Call(t.StarThread, t.transformFn, starlark.Tuple{nil, nil}, nil)
	if err != nil {
		switch err := err.(type) {
...
func (t *Starlark) executeDetect(fn *starlark.Function, dir string) (services map[string][]transformertypes.Artifact, err error) {
	...
	_, err = starlark.Call(t.StarThread, fn, starlark.Tuple{}, nil)
	if err != nil {
...
func (t *Starlark) getStarlarkEvalTemplate() *starlark.Builtin {
	return starlark.NewBuiltin(evalTemplate, func(thread *starlark.Thread, fn *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
		...
		for _, x := range data.Items() {
			logrus.Infof("TODO: getStarlarkEvalTemplate x: %+v", x)
